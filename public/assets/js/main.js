...
function contact() {
    ...
    request('mail.php', '#contact-form', (data) => {
        try {
            let formMessage = document.getElementById('form-response');
            data = JSON.parse(data);
            if(data instanceof Array) {
                for(var i = 0; i < data.length; ++i) {
                    switch(i) {
                        case 0: {formMessage.innerHTML += 'Message sent successfully'; break;}
                        case 1: {formMessage.innerHTML += 'All fields required'; break;}
                        case 2: {formMessage.innerHTML += 'Something went wrong! Message not sent'; break;}
                        default: formMessage.innerHTML += 'Unknown error occured!';
                    }
                }
            }
            else {
